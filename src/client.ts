...
export class Client<Ctx extends unknown = null> {
  ...
  public openChannel = (options: ChannelOptions<Ctx>, cb: OpenChannelCb<Ctx>): (() => void) => {
    ...
    if (this.connectionState === ConnectionState.CONNECTED) {
      ...
      if (
        !this.channelRequests.some(
          (cr) => cr !== channelRequest && cr.options.name === options.name,
        )
      ) {
        // There is an inflight channel that's closing
        // it will open our channel once its done closing.
